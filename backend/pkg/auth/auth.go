...
type MeHandlerOptions struct {
	...
	GroupsPaths string
	// VerifyToken, when set, is invoked before decoding the token claims. It should return
	// a non-zero HTTP status and error when verification fails.
	VerifyToken func(ctx context.Context, r *http.Request, clusterName, token string) (int, error)
}
...
func HandleMe(opts MeHandlerOptions) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if opts.VerifyToken != nil {
			if status, err := opts.VerifyToken(r.Context(), r, clusterName, token); err != nil {
				if status == 0 {
		...
		case []interface{}:
			...
			for _, it := range v {
				...
				case int64:
					out = append(out, fmt.Sprintf("%v", s))
				default:
					if encoded, ok := marshalToString(s); ok && encoded != "" {
